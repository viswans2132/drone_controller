...
namespace drone_controller{
	...
	:nh_(nh), private_nh_(private_nh){
		...
		pose_sub_ = nh_.subscribe("pose_msg", 1, 
                            &AsmNode::PoseCallback, this);
		odom_sub_ = nh_.subscribe("odom_msg", 1, 
                            &AsmNode::OdomCallback, this);
		traj_sub_ = nh_.subscribe("traj_msg", 1, 
                            &AsmNode::TrajCallback, this);
		mavState_sub_ = nh_.subscribe("mavros/state", 1, 
							&AsmNode::MavStateCallback, this, ros::TransportHints().tcpNoDelay());

        pub_timer_ = nh_.createTimer(ros::Duration(0.01), &AsmNode::PubCallback, this,
			false);
        com_timer_ = nh_.createTimer(ros::Duration(0), &AsmNode::ComCallback, this,
			true, false);
        sts_timer_ = nh_.createTimer(ros::Duration(0), &AsmNode::StsCallback, this, false);
        mavCom_timer_ = nh_.createTimer(ros::Duration(0), &AsmNode::MavComCallback, this, false);
	...
	void AsmNode::initParams(){
		...
		GetParameterArray(private_nh_, &position_controller_.theta_p_, "control/theta_p", Eigen::Vector3d(1.0, 1.0, 1.0));
		GetParameterArray(private_nh_, &position_controller_.theta_q_, "control/theta_q", Eigen::Vector3d(1.0, 1.0, 1.0));
	...
	void AsmNode::PoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg){
	...
	void AsmNode::OdomCallback(const nav_msgs::OdometryConstPtr& msg){
		EigenOdometry odometry;
		eigenOdometryFromMsg(msg, &odometry);
		position_controller_.setOdometry(odometry);
	}

	void AsmNode::TrajCallback(const trajectory_msgs::MultiDOFJointTrajectoryConstPtr& msg){
	...
	void AsmNode::ComCallback(const ros::TimerEvent &e){	
	...
	void AsmNode::PubCallback(const ros::TimerEvent &e){
	...
	void AsmNode::StsCallback(const ros::TimerEvent &e){
	...
	void AsmNode::MavComCallback(const ros::TimerEvent &e){
		switch (node_state_) {
	...
	void AsmNode::MavStateCallback(const mavros_msgs::State::ConstPtr &msg) { 
		current_state_ = *msg;
	}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "asm_node");

  ros::NodeHandle nh;
  ros::NodeHandle private_nh("~");
  drone_controller::AsmNode asm_node(nh, private_nh);
